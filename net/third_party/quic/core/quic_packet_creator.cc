...
QuicPacketCreator::~QuicPacketCreator() {
  ...
  DeleteFrames(&packet_.unreliable_frames);
}
...
bool QuicPacketCreator::ConsumeData(QuicStreamId id,
  ...
  CreateStreamFrame(id, write_length, iov_offset, offset, fin, unreliable, frame);
...
void QuicPacketCreator::CreateStreamFrame(QuicStreamId id,
  ...
  if (iov_offset == write_length) {
    ...
    *frame =
        QuicFrame(new QuicStreamFrame(id, /*fin*/true, /*unrel*/unreliable, offset, QuicStringPiece()));
    return;
  }
  ...
  *frame = QuicFrame(new QuicStreamFrame(id, set_fin, unreliable, offset, bytes_consumed));
}
...
void QuicPacketCreator::ClearPacket() {
  ...
  needs_full_padding_ = false;
  packet_.set_to_reliable = false;
  packet_.unreliable = false;
  DCHECK(packet_.unreliable_frames.empty());
  packet_.fake_acked = false;
}
...
void QuicPacketCreator::CreateAndSerializeStreamFrame(
  ...
  #ifdef SLST_DGB
 std::cerr  << "CreateAndSerializeStreamFrame" <<  std::endl;
 #endif
  DCHECK(queued_frames_.empty());
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "packing STREAM packet with frames: header: " << header <<  std::endl; 
 #endif
  ...
  frame = QuicMakeUnique<QuicStreamFrame>(id, set_fin, unreliable, stream_offset,
                                          bytes_consumed);
  #ifdef SLST_DEBUG 
 std::cout  << ENDPOINT << "Adding frame: " << *frame <<  std::endl; 
 #endif
...
#ifdef SLST_DBG
  std::cerr << "  Packet: " << packet_number() << "\n";
  std::cerr << "  Frame: " << *frame;
#endif
  if (!unreliable || set_fin) {
#ifdef SLST_DBG
    std::cerr << "  Marking frame as retransmittable.\n";
#endif
    packet_.retransmittable_frames.push_back(QuicFrame(frame.release()));
    packet_.unreliable = false;
  } else {
    packet_.unreliable_frames.push_back(QuicFrame(frame.release()));
    packet_.unreliable = true;
  }
#ifdef SLST_DBG
  std::cerr << std::endl;
#endif
...
bool QuicPacketCreator::HasPendingStreamFramesOfStream(QuicStreamId id) const {
  for (const auto& frame : packet_.retransmittable_frames) {
    if (frame.type == STREAM_FRAME && frame.stream_frame->stream_id == id) {
      return !((frame.type == STREAM_FRAME) ? (frame.stream_frame->unreliable && !frame.stream_frame->fin) : false);
    }
  }
  return false;
}
...
void QuicPacketCreator::FillPacketHeader(QuicPacketHeader* header) {
  ...
  #ifdef SLST_DEBUG 
 std::cout  << " NEWEST PACKET NUMBER: " << header->packet_number <<  std::endl; 
 #endif
...
bool QuicPacketCreator::AddFrame(const QuicFrame& frame,
  ...
  bool is_unreliable = (frame.type == STREAM_FRAME) ? (frame.stream_frame->unreliable && !frame.stream_frame->fin) : false;

#ifdef SLST_DBG
  std::cerr << "AddFrame: frame:" << frame;
#endif
  #ifdef SLST_DEBUG 
 std::cout  << ENDPOINT << "QuicPacketCreator::AddFrame: (send):" << frame <<  std::endl; 
 #endif
/*
  if (frame.type == STREAM_FRAME) {
     #ifdef SLST_DEBUG 
 std::cout  << "creating stream frame: (unrel?):" << frame.stream_frame->unreliable << " with data: ";
    for(int i = 0; i < frame.stream_frame->data_length; ++i)
      std::cout << std::hex << (int)frame.stream_frame->data_buffer[i];
         
    std::cout << std::dec << "\nplain: " << frame.stream_frame->data_buffer <<  std::endl; 
 #endif
...
*/
  if (frame.type == STREAM_FRAME &&
    ...
    //is_unreliable = frame.stream_frame->unreliable;
  ...
  // packet_number() returns the number of the last _created_ packet, so we are currently adding frames to the packet
  // that is created _next_.
#ifdef SLST_DBG
  std::cerr << "AddFrame: packet: " << packet_number() + 1 << std::endl;
#endif
  if (!is_unreliable && save_retransmittable_frames && ShouldRetransmit(frame)) {
...
#ifdef SLST_DBG
    std::cerr << "          Marked frame as retransmittable: " << frame;
    std::cerr << "          Current retransmittable frame queue:\n";
    for (const auto& frame : packet_.retransmittable_frames) {
      std::cerr << "            " << frame;
    }
#endif
  ...
  } else if (save_retransmittable_frames && ShouldRetransmit(frame))  {
    if (packet_.unreliable_frames.empty()) {
      packet_.unreliable_frames.reserve(2);
    }
      packet_.unreliable_frames.push_back(frame);
#ifdef SLST_DBG
      std::cerr << "          Marked frame as unreliable: " << frame;
#endif
  }
  queued_frames_.push_back(frame);
#ifdef SLST_DBG
  std::cerr << "          Current frame queue:\n";
  for (const auto& frame : queued_frames_) {
    std::cerr << "            " << frame;
  }
  std::cerr << std::endl;
#endif

  // Do not change the packet if it was explicitly set to reliable once.
  if (!packet_.set_to_reliable) {
    if (!is_unreliable) {
      // First time seeing a reliable frame.
      packet_.set_to_reliable = true;
    }
    packet_.unreliable = is_unreliable;
  }
