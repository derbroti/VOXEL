...
QuicSpdyClientStream* QuicSpdyClientSession::CreateOutgoingDynamicStream() {
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "CreateOutgoingDynamicStream with id: " << stream->id() << " unrel: " << stream->get_unreliable() <<  std::endl; 
 #endif
...
QuicSpdyStream* QuicSpdyClientSession::CreateIncomingDynamicStream(
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "CREATED INCOMING STREAM with id: " << stream->id() <<  std::endl; 
 #endif
