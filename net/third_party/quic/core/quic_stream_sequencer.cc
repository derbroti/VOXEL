...
QuicStreamSequencer::QuicStreamSequencer(QuicStream* quic_stream)
    : stream_(quic_stream),
      ...
      unreliable_buffered_(false) {}

QuicStreamSequencer::~QuicStreamSequencer() {}

void QuicStreamSequencer::OnStreamFrame(const QuicStreamFrame& frame) {
#ifdef SLST_DBG
  std::cerr << "QuicStreamSequencer::OnStreamFrame: frame:" << frame << std::endl;
#endif
  ...
  #ifdef SLST_DEBUG
  std::cerr << "frame: fin: " << frame.fin << " buff_unrel: " << unreliable_buffered_ << "f.unrel: " << frame.unreliable << std::endl;
  #endif
  
  if (frame.fin) {
    #ifdef SLST_DEBUG 
    std::cout << "QuicStreamSequencer::OnStreamFrame: fin: 1: off: " << frame.offset + data_len << std::endl;
    #endif
  ...
  QuicErrorCode result = buffered_frames_.OnStreamData(
      ...
      &bytes_written, &error_details, frame.unreliable, frame.get_receipt_time(),
      &padded);

  if (padded > 0)
    stream_->stream_bytes_read_adjust(padded);
...
bool QuicStreamSequencer::HasBytesToRead() const {
  //NOTE: change debug string to avoid recursion
  //#ifdef SLST_DEBUG 
...
bool QuicStreamSequencer::FillHoles() {
  //MARKER
  //return true; 
  return buffered_frames_.FillHoles();
};

bool QuicStreamSequencer::IsClosed() const {
   #ifdef SLST_DEBUG 
  if (close_offset_ != 18446744073709551615ull)
    std::cout << "IsClosed: cons: " << buffered_frames_.BytesConsumed() << " vs " << close_offset_ << std::endl;
  #endif
