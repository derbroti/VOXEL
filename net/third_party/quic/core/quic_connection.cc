...
QuicConnection::QuicConnection(
  ...
  std::cerr << "[connection] id:" << connection_id << " cct:" << stats_.connection_creation_time.ToDebuggingValue() << std::endl;
...
bool QuicConnection::OnAckRange(QuicPacketNumber start,
  ...
  //QUIC_DVLOG(1)
  #ifdef SLST_DEBUG 
 std::cout  << "OnAckRange: [" << start << ", " << end
                << "), last_range: " << last_range <<  std::endl; 
 #endif
...
QuicConsumedData QuicConnection::SendStreamData(QuicStreamId id,
  ...
  return packet_generator_.ConsumeData(id, write_length, offset, state, unreliable);
}
...
void QuicConnection::OnStreamReset(QuicStreamId id,
  ...
  while (packet_iterator != queued_packets_.end()) {
    ...
    QuicFrames* unreliable_frames =
        &packet_iterator->unreliable_frames;
    if (retransmittable_frames->empty() && unreliable_frames->empty()) {
      ++packet_iterator;
      continue;
    }
    RemoveFramesForStream(unreliable_frames, id);
...
void QuicConnection::ProcessUdpPacket(const QuicSocketAddress& self_address,
  ...
  if (!framer_.ProcessPacket(packet, packet.receipt_time())) {
...
bool QuicConnection::WritePacket(SerializedPacket* packet) {
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "WRITE packet: " <<  packet->packet_number << "\n" <<  std::endl; 
 #endif
...
bool QuicConnection::ShouldDiscardPacket(const SerializedPacket& packet) {
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "PACKET NR: " << packet_number <<  std::endl; 
 #endif
...
void QuicConnection::SendOrQueuePacket(SerializedPacket* packet) {
  ...
  if (!queued_packets_.empty() || !WritePacket(packet)) {
    ...
    packet->unreliable_frames.clear();
  }
...
#ifdef SLST_DBG
  std::cerr << "OnRetransmissionTimeout" << std::endl << std::endl;
#endif
...
#ifdef SLST_DBG
    std::cerr << "Rearming retransmission alarm" << std::endl << std::endl;
#endif
    SetRetransmissionAlarm();
  } else {
#ifdef SLST_DBG
    std::cerr << "NOT rearming retransmission alarm" << std::endl << std::endl;
#endif
  }
}
...
void QuicConnection::MaybeProcessUndecryptablePackets() {
  ...
  while (connected_ && !undecryptable_packets_.empty()) {
    ...
    if (!framer_.ProcessPacket(*packet, QuicTime::Zero()) &&
