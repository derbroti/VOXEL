...
QuicUnackedPacketMap::~QuicUnackedPacketMap() {
  for (QuicTransmissionInfo& transmission_info : unacked_packets_) {
    ...
    DeleteFrames(&(transmission_info.unreliable_frames));
  }
}
...
void QuicUnackedPacketMap::AddSentPacket(SerializedPacket* packet,
  ...
  info.unreliable = packet->unreliable;
  info.fake_acked = packet->fake_acked;
...
#ifdef SLST_DBG
  std::cerr << "QuicUnackedPacketMap::AddSentPacket: packet: " << packet_number << " unrel: " << packet->unreliable << std::endl;
#endif
  ...
  if (old_packet_number == 0) {
    ...
    packet->unreliable_frames.swap(
        unacked_packets_.back().unreliable_frames);
  }
}
...
void QuicUnackedPacketMap::RemoveObsoletePackets() {
  while (!unacked_packets_.empty()) {
    ...
    if (unacked_packets_.front().fake_acked && !unacked_packets_.front().loss_considered) {
      break;
    }
    if (session_decides_what_to_write_) {
      DeleteFrames(&unacked_packets_.front().retransmittable_frames);
      DeleteFrames(&unacked_packets_.front().unreliable_frames);
    }
...
void QuicUnackedPacketMap::MarkLossConsidered(QuicPacketNumber packet_number) {
  DCHECK_GE(packet_number, least_unacked_);
  unacked_packets_[packet_number - least_unacked_].loss_considered = true;
}
...
void QuicUnackedPacketMap::TransferRetransmissionInfo(
  ...
  info->unreliable_frames.swap(transmission_info->unreliable_frames);
...
void QuicUnackedPacketMap::RemoveRetransmittability(
  ...
  if (session_decides_what_to_write_) {
    ...
    DeleteFrames(&info->unreliable_frames);
    return;
  }
...
void QuicUnackedPacketMap::CancelRetransmissionsForStream(
  ...
  for (UnackedPacketMap::iterator it = unacked_packets_.begin();
    ...
    QuicFrames* unreliable_frames = &it->unreliable_frames;
    if (!unreliable_frames->empty()) {
      RemoveFramesForStream(unreliable_frames, stream_id);
    }
    if (frames->empty()) {
      continue;
    }
    RemoveFramesForStream(frames, stream_id);
    if (frames->empty()) { //FIXME Maybe
...
bool QuicUnackedPacketMap::NotifyFramesAcked(const QuicTransmissionInfo& info,
  ...
  for (const QuicFrame& frame : info.unreliable_frames) {
    if (session_notifier_->OnFrameAcked(frame, ack_delay)) {
      new_data_acked = true;
    }
  }
