...
class QUIC_EXPORT_PRIVATE QuicStream {
 public:
  ...
  // Supposed to be called by subclass to decode fec'd data if needed
  ...
  void stream_bytes_read_adjust(uint64_t amount) { stream_bytes_read_ += amount; }
  ...
  bool get_unreliable() { return unreliable_; }

  void set_fec(uint8_t fec) { fec_ = fec; }
  uint8_t get_fec() { return fec_; }
 ...
 protected:
  ...
  // True if the stream is unreliable
  bool unreliable_;

  //defines the amount of redundancy added
  uint8_t fec_;
