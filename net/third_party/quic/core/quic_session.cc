...
void QuicSession::OnStreamFrame(const QuicStreamFrame& frame) {
  ...
  QuicStream* stream = GetOrCreateStream(stream_id, frame.unreliable);
  if (!stream) {
...
void QuicSession::OnRstStream(const QuicRstStreamFrame& frame) {
  ...
  QuicStream* stream = GetOrCreateDynamicStream(stream_id, /*unrel,nochange*/2);
  if (!stream) {
...
QuicConsumedData QuicSession::WritevData(QuicStream* stream,
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "WritevData call" <<  std::endl; 
 #endif
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "WritevData: stream is unrel: " << stream->get_unreliable() << " on: " << id << "|" << stream->id() <<  std::endl; 
 #endif
  QuicConsumedData data =
      connection_->SendStreamData(id, write_length, offset, state, stream->get_unreliable());
...
void QuicSession::CloseStreamInner(QuicStreamId stream_id, bool locally_reset) {
   #ifdef SLST_DEBUG 
  std::cout << ENDPOINT << "CloseStreamInner: id: " << stream_id << std::endl;
  #endif
...
void QuicSession::ActivateStream(std::unique_ptr<QuicStream> stream) {
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "ActivateStream: " << stream_id << " unrel: " << stream->get_unreliable() <<  std::endl; 
 #endif
...
QuicStream* QuicSession::GetOrCreateStream(const QuicStreamId stream_id, bool unreliable) {
  StaticStreamMap::iterator it = static_stream_map_.find(stream_id);
  if (it != static_stream_map_.end()) {
    return it->second;
  }
  return GetOrCreateDynamicStream(stream_id, unreliable);
}
...
QuicStream* QuicSession::GetOrCreateStream(const QuicStreamId stream_id) {
  ...
  return GetOrCreateDynamicStream(stream_id, /*unrel,nochange*/2);
}
...
QuicStream* QuicSession::GetOrCreateDynamicStream(
    const QuicStreamId stream_id, int unreliable) {
  DCHECK(!QuicContainsKey(static_stream_map_, stream_id))
  ...
  if (it != dynamic_stream_map_.end()) {
    #ifdef SLST_DEBUG 
 std::cout  << "FOUND stream for id: " << stream_id << " unrel: " << it->second.get()->get_unreliable() <<  std::endl; 
 #endif
  ...
  auto* stream = CreateIncomingDynamicStream(stream_id);
  if (unreliable < 2)
    stream->set_unreliable(unreliable==1?true:false);
  #ifdef SLST_DEBUG 
 std::cout  << "CREATED dynamic INCOMING with id: " << stream_id << " unrel: " << stream->get_unreliable() <<  std::endl; 
 #endif
  return stream;
}
...
bool QuicSession::OnFrameAcked(const QuicFrame& frame,
  ...
  if (stream != nullptr) {
    new_stream_data_acked = stream->OnStreamFrameAcked(
        ...
        frame.stream_frame->fin, frame.stream_frame->unreliable, ack_delay_time);
...
#ifdef SLST_DBG
  std::cerr << "OnStreamFrameRetransmitted: frame:" << frame << std::endl;
#endif
...
#ifdef SLST_DBG
  std::cerr << "OnFrameLost: frame:" << frame << std::endl;
#endif
  if (frame.type != STREAM_FRAME) {
