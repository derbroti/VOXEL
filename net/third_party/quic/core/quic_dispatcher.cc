...
class PacketCollector : public QuicPacketCreator::DelegateInterface,
 ...
 public:
  ...
  explicit PacketCollector(QuicBufferAllocator* allocator)
      : dummy_lost(false), send_buffer_(allocator) {}
  ...
  void OnSerializedPacket(SerializedPacket* serialized_packet) override {
    ...
    DeleteFrames(&(serialized_packet->unreliable_frames));
    serialized_packet->unreliable_frames.clear();
  }
  ...
  bool WriteStreamData(QuicStreamId id,
    ...
    #ifdef SLST_DEBUG 
 std::cout  << "id: " << id << " offset: " << offset << " lost?: " << dummy_lost <<  std::endl; 
 #endif
   /*
    if (id == 5 && offset >= 89027 and dummy_lost == false) {
	    #ifdef SLST_DEBUG 
 std::cout  << "will lose frame on stream id=5 with offset >= 89027" <<  std::endl; 
 #endif
	    dummy_lost = true;
	    return true;
    }
...
void QuicDispatcher::ProcessPacket(const QuicSocketAddress& self_address,
  ...
  framer_.ProcessPacket(packet, packet.receipt_time());
