...
bool QuicSpdyStream::FillHoles() {
  return sequencer()->FillHoles();
}
...
void QuicSpdyStream::OnStreamHeaderList(bool fin,
  ...
  if (!headers_decompressed_) {
    OnInitialHeadersComplete(fin, unreliable, frame_len, header_list);
  } else {
    OnTrailingHeadersComplete(fin, unreliable, frame_len, header_list);
  }
}
...
void QuicSpdyStream::OnInitialHeadersComplete(
  ...
  if (fin) {
    OnStreamFrame(QuicStreamFrame(id(), fin, unreliable, 0, QuicStringPiece()));
  }
  if (FinishedReadingHeaders()) {
    sequencer()->SetUnblocked();
  }
...
void QuicSpdyStream::OnTrailingHeadersComplete(
  ...
  OnStreamFrame(
      QuicStreamFrame(id(), fin, unreliable, final_byte_offset, QuicStringPiece()));
}
