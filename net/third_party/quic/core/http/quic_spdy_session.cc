...
class QuicSpdySession::SpdyFramerVisitor
 ...
 public:
  ...
  void OnHeaders(SpdyStreamId stream_id,
    ...
    session_->OnHeaders(stream_id, has_priority, priority, fin, unreliable);
  }
  ...
  void OnPushPromise(SpdyStreamId stream_id,
    ...
    session_->OnPushPromise(stream_id, promised_stream_id, end, unreliable);
  }
  ...
  void OnPriority(SpdyStreamId stream_id,
    ...
    session_->OnPriority(stream_id, priority, unreliable);
  }
...
void QuicSpdySession::OnStreamHeadersPriority(QuicStreamId stream_id,
                                              SpdyPriority priority,
                                              bool unreliable) {
  QuicSpdyStream* stream = GetSpdyDataStream(stream_id, unreliable);
  if (!stream) {
...
void QuicSpdySession::OnStreamHeaderList(QuicStreamId stream_id,
  ...
  stream->OnStreamHeaderList(fin, unreliable, frame_len, header_list);
}
...
QuicSpdyStream* QuicSpdySession::GetSpdyDataStream(
    const QuicStreamId stream_id, bool unreliable) {
  return static_cast<QuicSpdyStream*>(GetOrCreateDynamicStream(stream_id, unreliable));
}
...
void QuicSpdySession::OnHeaders(SpdyStreamId stream_id,
  ...
  if (has_priority) {
    ...
    OnStreamHeadersPriority(stream_id, priority, unreliable);
  } else {
    if (perspective() == Perspective::IS_SERVER) {
      CloseConnectionWithDetails(QUIC_INVALID_HEADERS_STREAM_DATA,
...
void QuicSpdySession::OnPriority(SpdyStreamId stream_id,
  ...
  OnPriorityFrame(stream_id, priority, unreliable);
}
...
void QuicSpdySession::OnHeaderList(const QuicHeaderList& header_list) {
  ...
  if (promised_stream_id_ == kInvalidStreamId) {
    OnStreamHeaderList(stream_id_, fin_, unreliable_, frame_len_, header_list);
  } else {
    OnPromiseHeaderList(stream_id_, promised_stream_id_, frame_len_,
                        header_list, unreliable_);
  }
