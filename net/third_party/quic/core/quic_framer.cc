...
size_t QuicFramer::BuildDataPacket(const QuicPacketHeader& header,
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "packing packet with frames: header: " << header <<  std::endl; 
 #endif
 ...
 std::cout  << ": " << frame <<  std::endl; 
    ...
    /*
	  if (frame.type == STREAM_FRAME && (*(frame).stream_frame).stream_id == 5 && (*(frame).stream_frame).offset >= 89027 && kill_once == true) {
		kill_once = false;
		#ifdef SLST_DEBUG 
 std::cout  << "you want to kill: " <<  header <<  std::endl; 
 #endif
	  }	
    */
...
size_t QuicFramer::BuildIetfDataPacket(const QuicPacketHeader& header,
                                       ...
                                       size_t packet_length) {
  #ifdef SLST_DEBUG 
 std::cout  << "packing IETF packet with frames: header: " << header <<  std::endl; 
 #endif
  QuicDataWriter writer(packet_length, buffer, endianness());
...
bool QuicFramer::ProcessPacket(const QuicEncryptedPacket& packet, QuicTime qt) {
  ...
  if (IsVersionNegotiation(header)) {
    ...
    if (last_packet_is_ietf_quic_) {
      ...
      rv = ProcessDataPacket(&reader, &header, packet, buffer, kMaxPacketSize, qt);
    }
  } else {
...
bool QuicFramer::ProcessIetfDataPacket(QuicDataReader* encrypted_reader,
                                       ...
                                       size_t buffer_length, QuicTime qt) {
  ...
  if (version_.transport_version == QUIC_VERSION_99) {
    ...
    if (!ProcessFrameData(&reader, *header, qt)) {
...
bool QuicFramer::ProcessFrameData(QuicDataReader* reader,
                                  const QuicPacketHeader& header, QuicTime qt) {
  ...
  while (!reader->IsDoneReading()) {
    ...
    if (frame_type & kQuicFrameTypeSpecialMask) {
      ...
      if ((version_.transport_version != QUIC_VERSION_41 &&
        ...
        frame.set_receipt_time(qt);
	#ifdef SLST_DEBUG 
 std::cerr  << "RECEIVED: stream frame: " << frame << std::endl; //<< " with header: " << header << "\nwith data: ";
 #endif
        /*for(int i = 0; i < frame.data_length; ++i)
          std::cout << std::hex << (int)frame.data_buffer[i];
         
        std::cout << std::dec << "\nplain: " << frame.data_buffer <<  std::endl; 
      ...
      if ((version_.transport_version != QUIC_VERSION_41 &&
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED ack frame:\nwith header: " << header <<  std::endl; 
 #endif
        continue;
      }
    ...
    switch (frame_type) {
      case PADDING_FRAME: {
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED padding frame:" << frame << " with header: " << header <<  std::endl; 
 #endif
      ...
      case RST_STREAM_FRAME: {
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED rst frame:" << frame << " with header: " << header <<  std::endl; 
 #endif
      ...
      case CONNECTION_CLOSE_FRAME: {
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED conn close frame:" << frame << " with header: " << header <<  std::endl; 
 #endif
      ...
      case WINDOW_UPDATE_FRAME: {
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED wnd update frame:" << window_update_frame << " with header: " << header <<  std::endl; 
 #endif
      ...
      case BLOCKED_FRAME: {
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED blocked frame:" << blocked_frame << " with header: " << header <<  std::endl; 
 #endif
      ...
      case STOP_WAITING_FRAME: {
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED stop waiting frame:" << stop_waiting_frame << " with header: " << header <<  std::endl; 
 #endif
      ...
      case PING_FRAME: {
        ...
        #ifdef SLST_DEBUG 
 std::cout  << "RECEIVED ping frame:" << ping_frame << " with header: " << header <<  std::endl; 
 #endif
...
bool QuicFramer::ProcessStreamFrame(QuicDataReader* reader,
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "recv A stream_id is: " << std::hex  << stream_id << std::dec <<  std::endl; 
 #endif

  if ((stream_id & (1 << (stream_id_length*8 - 1))) != 0) {
    frame->unreliable = true;
    #ifdef SLST_DEBUG 
 std::cout  << "unreliable processing stream frame" <<  std::endl; 
 #endif
  } else {
    frame->unreliable = false;
    #ifdef SLST_DEBUG 
 std::cout  << "reliable processing stream frame" <<  std::endl; 
 #endif
  }
  stream_id &= ~(1 << (stream_id_length*8 - 1));

  #ifdef SLST_DEBUG 
 std::cout  << "recv B stream_id is: " << std::hex  << stream_id << std::dec <<  std::endl; 
 #endif
...
bool QuicFramer::AppendStreamId(size_t stream_id_length,
  ...
  if (unreliable) {
	  #ifdef SLST_DEBUG 
 std::cout  << "AppendStreamId for UNRELIALBE stream_frame" <<  std::endl; 
 #endif
    #ifdef SLST_DEBUG 
 std::cout  << "send A stream_id is: " << std::hex  << stream_id << std::dec <<  std::endl; 
 #endif
    stream_id |= 1 << (stream_id_length*8-1);
    #ifdef SLST_DEBUG 
 std::cout  << "send B stream_id is: " << std::hex  << stream_id << std::dec <<  std::endl; 
 #endif
  }
  else
  {
    #ifdef SLST_DEBUG 
 std::cout  << "AppendStreamId for reliable stream_frame" <<  std::endl; 
 #endif
    #ifdef SLST_DEBUG 
 std::cout  << "send X stream_id is: " << std::hex  << stream_id << std::dec <<  std::endl; 
 #endif
  }
