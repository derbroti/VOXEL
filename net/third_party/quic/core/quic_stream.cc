...
QuicStream::QuicStream(QuicStreamId id, QuicSession* session, bool is_static)
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "QuicStream() id: " << id_ << std::endl; 
 #endif
}
...
void QuicStream::OnStreamFrame(const QuicStreamFrame& frame) {
  ...
  set_unreliable(frame.unreliable);
...
void QuicStream::WriteOrBufferData(
   ...
   #ifdef SLST_DEBUG 
 std::cout  << "WriteOrBufferData id: " << id() << " unrel: " << this->unreliable_ << " this: " << this << " tid: " << std::this_thread::get_id() << std::endl; 
 #endif
  #ifdef SLST_DEBUG 
 std::cout  << "WriteOrBufferData am i unreliable?: " << unreliable_  << " id: " << id() <<  " this: " << this << std::endl; 
 #endif
  ...
  if (data.length() > 0) {
    ...
    #ifdef SLST_DEBUG 
 std::cout  << "WriteOrBufferData before OnDataBuffered: id: " << id() << " data-len: " << data.length() << " offset:" << send_buffer_.stream_offset() << " had_buffered_data: " << had_buffered_data << std::endl; 
 #endif
  ...
  if (!had_buffered_data && (HasBufferedData() || fin_buffered_)) {
    ...
    #ifdef SLST_DEBUG 
 std::cout  << "WriteOrBufferData before WriteBufferedData: id: " << id() << " data-len: " << data.length() << " offset:" << send_buffer_.stream_offset() << " had_buffered_data: " << had_buffered_data << std::endl; 
 #endif
    WriteBufferedData();
  }
}
...
QuicConsumedData QuicStream::WritevDataInner(size_t write_length,
  ...
  #ifdef SLST_DEBUG 
 std::cout  << "WritevDataInner on id: " << id()  << " fin: " << fin << std::endl; 
 #endif
...
void QuicStream::CloseReadSide() {
  ...
  QUIC_DVLOG(1) << ENDPOINT << "Done reading from stream " << id();
   #ifdef SLST_DEBUG 
  std::cout << "CloseReadSide: id: " << id() << std::endl;
  #endif
...
void QuicStream::CloseWriteSide() {
  ...
  QUIC_DVLOG(1) << ENDPOINT << "Done writing to stream " << id();
   #ifdef SLST_DEBUG 
  std::cout << "CloseWriteSide: id: " << id() << std::endl;
  #endif
...
void QuicStream::decode_data() {
  //nothing will be done
  std::cerr << " should not be called from base class..." << std::endl;
}
...
bool QuicStream::OnStreamFrameAcked(QuicStreamOffset offset,
  ...
  #ifdef SLST_DEBUG 
 std::cout << ENDPOINT << "stream " << id_ << " Acking "
                << "[" << offset << ", " << offset + data_length << "]"
                << " fin = " << fin_acked << std::endl;
 #endif
...
void QuicStream::WriteBufferedData() {
  ...
  #ifdef SLST_DEBUG
  std::cout << "WriteBufferedData() " << std::endl;
  #endif
