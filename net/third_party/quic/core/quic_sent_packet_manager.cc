...
void QuicSentPacketManager::MaybeInvokeCongestionEvent(
    ...
    QuicTime event_time) {
  AckedPacketVector real_packets_acked;
  for (auto packet : packets_acked_) {
    auto &transinfo = unacked_packets_.GetTransmissionInfo(packet.packet_number);
    if (!transinfo.fake_acked) {
      real_packets_acked.push_back(packet);
    }
  }
  if (!rtt_updated && real_packets_acked.empty() && packets_lost_.empty()) {
    return;
  }
  if (using_pacing_) {
    pacing_sender_.OnCongestionEvent(rtt_updated, prior_in_flight, event_time,
                                     real_packets_acked, packets_lost_);
  } else {
...
void QuicSentPacketManager::RetransmitUnackedPackets(
...
#ifdef SLST_DBG
  std::cerr << "RetransmitUnackedPackets: retranmission_type:"
            << QuicUtils::TransmissionTypeToString(retransmission_type) << std::endl << std::endl;
#endif
  ...
  for (QuicUnackedPacketMap::const_iterator it = unacked_packets_.begin();
    ...
    if ((retransmission_type == ALL_UNACKED_RETRANSMISSION ||
	    ...
	    #ifdef SLST_DEBUG 
 std::cout  << "marking packet for retransmission: " << packet_number <<  std::endl; 
 #endif
...
void QuicSentPacketManager::MarkForRetransmission(
  ...
  QuicTransmissionInfo* transmission_info =
	...
	#ifdef SLST_DEBUG 
 std::cout  << " potentially! mark for retrans: " << packet_number <<  std::endl; 
 #endif

#ifdef SLST_DBG
  std::cerr << "MarkForRetransmission: packet_number:" << packet_number << " transmission_type:"
            << QuicUtils::TransmissionTypeToString(transmission_type) << std::endl;
  std::cerr << "  Retransmittable frames in packet:\n";
#endif
  for (const QuicFrame& frame : transmission_info->retransmittable_frames) {
#ifdef SLST_DBG
    std::cerr << "    " << frame;
#endif
    if (frame.type == STREAM_FRAME){
      found = true;
      if (!frame.stream_frame->unreliable || frame.stream_frame->fin) {
#ifdef SLST_DBG
          std::cerr << "      Found RELIABLE frame! Breaking.\n";
#endif
          pure = false;
          #ifdef SLST_DEBUG 
 std::cout  << "packet not pure... DO mark!" <<  std::endl; 
 #endif
          break;
      }
    }
  ...
  if (session_decides_what_to_write()) {
    if (transmission_type == HANDSHAKE_RETRANSMISSION ||
      ...
      {
#ifdef SLST_DBG
        std::cerr << "  Calling RetransmitFrames\n";
#endif
        unacked_packets_.RetransmitFrames(*transmission_info, transmission_type);
        #ifdef SLST_DEBUG 
 std::cout  << "NOTE: we have outstanding (unpure) data!" << transmission_info->retransmittable_frames << "\n\n" <<  std::endl; 
 #endif
      }
    } else {
      //MARKER
      if (!pure) {
#ifdef SLST_DBG
        std::cerr << "  Calling NotifyFramesLost\n";
#endif
...
#ifdef SLST_DBG
      std::cerr << "  Changed transmission type to: "
                << QuicUtils::TransmissionTypeToString(transmission_type) << std::endl;
#endif
    }
    else {
#ifdef SLST_DBG
      std::cerr << "  Removing packet\n";
#endif
      //unacked_packets_.RemoveFromInFlight(packet_number);
      unacked_packets_.RemoveFromInFlight(transmission_info);
      unacked_packets_.RemoveRetransmittability(transmission_info);
...
bool QuicSentPacketManager::OnPacketSent(
...
#ifdef SLST_DBG
  std::cerr << "OnPacketSent: packet:" << packet_number << " original:" << original_packet_number << " transmission:"
  << QuicUtils::TransmissionTypeToString(transmission_type) << std::endl << std::endl;
#endif
  ...
  bool actually_in_flight = in_flight;
  if (!in_flight && serialized_packet->unreliable) {
    // Fake this so that the pacer and send algorithm account for the in-flight data.
    has_retransmittable_data = HAS_RETRANSMITTABLE_DATA;
    // Do NOT change the original in_flight bool, because we do not want to reset the retransmission timer for
    // unreliable packets?
    //FIXME
    actually_in_flight = true;
  }
  if (using_pacing_) {
  ...
  unacked_packets_.AddSentPacket(serialized_packet, original_packet_number,
                                 transmission_type, sent_time, actually_in_flight);
...
#ifdef SLST_DBG
  std::cerr << "MaybeRetransmitOldestPacket: type:" << QuicUtils::TransmissionTypeToString(type) << std::endl << std::endl;
#endif
  QuicPacketNumber packet_number = unacked_packets_.GetLeastUnacked();
  for (QuicUnackedPacketMap::const_iterator it = unacked_packets_.begin();
       it != unacked_packets_.end(); ++it, ++packet_number) {
...
void QuicSentPacketManager::OnAckFrameStart(QuicPacketNumber largest_acked,
                                            ...
                                            QuicTime ack_receive_time) {
	#ifdef SLST_DEBUG 
 std::cout  << "OnAckFrameStart: " << largest_acked <<  std::endl; 
 #endif
...
void QuicSentPacketManager::OnAckRange(QuicPacketNumber start,
...
#ifdef SLST_DBG
  std::cerr << "QuicSentPacketManager::OnAckRange: start:" << start << " end:" << end << std::endl;
  std::cerr << "  packets_acked_ size: " << packets_acked_.size() << std::endl;
#endif
  if (end > last_ack_frame_.largest_acked + 1) {
#ifdef SLST_DBG
    std::cerr << "  Increasing last_ack_frame.largest_acked from " << last_ack_frame_.largest_acked << " to " << end -1 << std::endl;
#endif
...
#ifdef SLST_DBG
  std::cerr << "  Current least_unacked: " << least_unacked << std::endl;
#endif
  ...
  for(QuicPacketNumber curr = start - 1; curr >= least_unacked; curr--) {
#ifdef SLST_DBG
    std::cerr << "  fake_acc for " << curr << "? ";
#endif
    auto insert_position = packets_acked_.begin();
    bool already_acked = false;
    for(; insert_position != packets_acked_.end(); insert_position++) {
      if ((*insert_position).packet_number == curr) {
        already_acked = true;
        break;
      } else if ((*insert_position).packet_number < curr) {
        break;
      }
    }
    if (already_acked) {
#ifdef SLST_DBG
      std::cerr << "already acked" << std::endl;
#endif
      continue;
    }
    auto *transinfo = unacked_packets_.GetMutableTransmissionInfo(curr);
    if (transinfo->unreliable && transinfo->state != ACKED) {
#ifdef SLST_DBG
      std::cerr << "yes";
#endif
      transinfo->fake_acked = true;
      if (insert_position == packets_acked_.end()) {
        packets_acked_.push_back(AckedPacket(curr, 0, QuicTime::Zero()));
      } else {
        packets_acked_.insert(insert_position, AckedPacket(curr, 0, QuicTime::Zero()));
      }
    } else {
#ifdef SLST_DBG
      std::cerr << "no";
...
#ifdef SLST_DBG
    std::cerr << " unrel:" << transinfo->unreliable << " state:" << QuicUtils::SentPacketStateToString(transinfo->state);
    std::cerr << std::endl;
#endif
  }
...
#ifdef SLST_DBG
  std::cerr << "  Setting start to: " << start << std::endl << std::endl;
#endif
  do {
    ...
    for (QuicPacketNumber acked = end - 1; acked >= newly_acked_start;
      ...
      bool already_acked = false;
      for(; insert_position != packets_acked_.end(); insert_position++) {
        if ((*insert_position).packet_number == acked) {
          auto *transinfo = unacked_packets_.GetMutableTransmissionInfo(acked);
#ifdef SLST_DBG
          std::cerr << std::endl;
          std::cerr << "    Removing fake ack " << acked << std::endl << "    ";
#endif
          transinfo->fake_acked = false;
          already_acked = true;
          break;
        } else if ((*insert_position).packet_number < acked) {
          break;
        }
      }
      if (already_acked) {
      ...
      if (insert_position == packets_acked_.end()) {
        ...
        std::cerr << " " << acked;
#endif
        packets_acked_.insert(insert_position, AckedPacket(acked, 0, QuicTime::Zero()));
      }
    }
#ifdef SLST_DBG
    std::cerr << std::endl;
...
#ifdef SLST_DBG
      std::cerr << std::endl;
      std::cerr << "  Post packets_acked_ size: " << packets_acked_.size() << std::endl;
#endif
      return;
    }
...
#ifdef SLST_DBG
  std::cerr << "  Post packets_acked_ size: " << packets_acked_.size() << std::endl << std::endl;
#endif
}

bool QuicSentPacketManager::OnAckFrameEnd(QuicTime ack_receive_time) {
#ifdef SLST_DBG
  std::cerr << "QuicSentPacketManager::OnAckFrameEnd: ack_receive_time:" << ack_receive_time.ToDebuggingValue() << std::endl;
  std::cerr << "  Unacked packets: " << unacked_packets_.GetNumUnackedPacketsDebugOnly() << std::endl;
  std::cerr << "  packets_acked_ size: " << packets_acked_.size() << std::endl;
#endif
