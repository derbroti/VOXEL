...
void TcpCubicSenderBytes::OnPacketLost(QuicPacketNumber packet_number,
  ...
  if (packet_number <= largest_sent_at_last_cutback_) {
    ...
    std::cerr << "[congestion] cct:" << stats_->connection_creation_time.ToDebuggingValue() << " type:ignore cw:" << congestion_window_ << " st:" << slowstart_threshold_ << std::endl;
    return;
  }
  ...
  std::cerr << "[congestion] cct:" << stats_->connection_creation_time.ToDebuggingValue() << " type:loss cw:" << congestion_window_ << " st:" << slowstart_threshold_ << std::endl;
}
...
void TcpCubicSenderBytes::MaybeIncreaseCwnd(
  ...
  if (InSlowStart()) {
    ...
    QUIC_DVLOG(1) << "Slow start; congestion window: " << congestion_window_
                  << " slowstart threshold: " << slowstart_threshold_;
    std::cerr << "[congestion] cct:" << stats_->connection_creation_time.ToDebuggingValue() << " type:slowstart cw:" << congestion_window_ << " st:" << slowstart_threshold_ << std::endl;
    return;
  }
  ...
  if (reno_) {
    ...
    std::cerr << "[congestion] cct:" << stats_->connection_creation_time.ToDebuggingValue() << " type:reno cw:" << congestion_window_ << " st:" << slowstart_threshold_ << std::endl;
  } else {
    ...
    QUIC_DVLOG(1) << "Cubic; congestion window: " << congestion_window_
                  << " slowstart threshold: " << slowstart_threshold_;
    std::cerr << "[congestion] cct:" << stats_->connection_creation_time.ToDebuggingValue() << " type:cubic cw:" << congestion_window_ << " st:" << slowstart_threshold_ << std::endl;
  }
}
