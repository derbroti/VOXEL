...
class QUIC_EXPORT_PRIVATE QuicSession : public QuicConnectionVisitorInterface,
 ...
 public:
  ...
  size_t num_active_requests() const { 
    //std::cerr << "n: " << dynamic_stream_map_.size() << " Z: " << zombie_streams_.size() << " r: ";
    /*
    for (auto it = dynamic_stream_map_.begin(); it != dynamic_stream_map_.end(); ++it )
    {
      std::cerr << (*it).first << " ";
    }*/
    //std::cerr << std::endl;
    return dynamic_stream_map_.size(); 
  }
  ...
  // returned.
  QuicStream* GetOrCreateStream(const QuicStreamId stream_id, bool unreliable);
 ...
 protected:
  ...
  // Caller does not own the returned stream.
  QuicStream* GetOrCreateDynamicStream(QuicStreamId stream_id, int unreliable);
